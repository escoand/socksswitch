...
int socksswitch_ssh_recv(LIBSSH2_CHANNEL ** channel, char *buf) {
    DEBUG;
...
int socksswitch_send(const int sock, const char *buf,
...
int socksswitch_ssh_send(LIBSSH2_CHANNEL ** channel, const char *buf,
			 const SOCKET_DATA_LEN len) {
    DEBUG;
    ...
    /* send data */
    while (bytessend < len) {
	rc = libssh2_channel_write(*channel, buf + bytessend,
				   len - bytessend);
...
int socksswitch_ssh_close(LIBSSH2_CHANNEL ** channel) {
...
int sshSocket(const char *host, const int port,
    ...
    *session = libssh2_session_init();
    ...
    if (rc != 0) {
	TRACE_WARNING
	    ("libssh2 session startup (session:%i err:%i): ",
	     *session, rc);
	sshError(*session);
	return SOCKET_ERROR;
    }
...
void sshError(LIBSSH2_SESSION * session) {
    int err = 0;
