...
void
getSocksReqHost (char *host, const char *buf, const int len)
{
  ...
  /* socks5 ipv4 */
  else if (req->ver == 5 && req->data.socks5.atyp == 1)
    {
      sprintf (host, "%i.%i.%i.%i", req->data.socks5.data.ipv4[0],
	       ...
	       req->data.socks5.data.ipv4[2], req->data.socks5.data.ipv4[3]);
    }
...
unsigned short
getSocksReqPort (const char *buf, const int len)
{
  SOCKS_REQUEST *req = (SOCKS_REQUEST *) malloc (len);
