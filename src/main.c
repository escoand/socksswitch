      ...
      for (i = 0; i < READ_SET_SIZE; i++)
	{
	  ...
	  if (FD_ISSET (FD_SET_DATA (read_set, i), &ssh_set))
	      ...
	      for (j = 0; j < sizeof (forwards) / sizeof (forwards[0]); j++)
		{
		  if (forwards[j].channel != NULL)
		    {
		      rc = socksswitch_ssh_recv (&forwards[i].channel, buf);
	  ...
	  /* socks5 handshake */
	  else if (rc == 3 && memcmp (buf, "\x05\x01\x00", 3) == 0)
	    {
	      DEBUG;
	      rc =
		socksswitch_send (FD_SET_DATA (read_set, i), "\x05\x00", 2);
	      if (rc < 0)
		cleanEnd (rc);
	    }
		  ...
		  if (newsock > 0)
		    {
		      ...
		      forwardsAdd (FORWARD_TYPE_DIRECT,
				   FD_SET_DATA (read_set, i), newsock, NULL);
...
/* read parameters */
void
readParams (const int argc, char *argv[])
{
  unsigned int i;
  char tmp[1025];
...
int
matchFilter (FORWARD_DESTINATION ** fo, const char *buf, const int len)
{
  unsigned int i, j;
  char host[256] = "";
...
int
forward (const int sock, LIBSSH2_CHANNEL * channel, const char *buf,
	 const int len)
{
  unsigned int i;
